/**
 * City of Oberlin Sync Pipeline — city_oberlin_v1
 *
 * Calls the City of Oberlin ingester, then runs every staged event through:
 ...
 *
 * Run: node --env-file=.env sync-city-of-oberlin.js
 */

import { initializeApp, cert, getApps } from "firebase-admin/app";
import { getFirestore } from "firebase-admin/firestore";
import { runIngester } from "./ingest-city-of-oberlin.js";
import {
  mightBeDuplicate,
...
const POST_TYPE_MAP = {
  ...
  "recreation": 12, "networking": 13, "council": 6,
  "meeting": 6, "hearing": 6, "commission": 6,
};
...
async function buildPayloadFromStaged(ev) {
  ...
  const payload = {
    ...
    sponsors:            [ev.organizational_sponsor || ev.source_name || "City of Oberlin"],
...
- City council meetings, public hearings, community events
...
async function loadProcessedFingerprints(db) {
  ...
  try {
    const [queueSnap, rejectedSnap] = await Promise.all([
      db.collection("review_queue").where("source_id", "==", "city_of_oberlin").select("fingerprint").get(),
      db.collection("rejected").where("source_id", "==", "city_of_oberlin").select("fingerprint").get(),
    ]);
    const fps = new Set();
...
async function fetchCommunityHubEvents() {
  try {
    const res = await fetch(COMMUNITYHUB_POSTS_API, {
      headers: { "User-Agent": "city-of-oberlin-sync-bot/1.0" },
    });
...
async function main() {
  ...
  console.log("→ Running City of Oberlin ingester…\n");
  ...
  for (let i = 0; i < stagedEvents.length; i++) {
    ...
    const alreadyQueued = await checkDuplicateInQueue(db, incoming, "city_of_oberlin");
  ...
  if (db) {
    await db.collection("syncs").doc("city_of_oberlin").set({
