...
const nav = [
  ...
  },
  {
    ...
    badge: "rejected",
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9 0 0 0 5.636 5.636m12.728 12.728A9 9 0 0 1 5.636 5.636m12.728 12.728L5.636 5.636" /></svg>,
  },
  {
    label: "Sources",
    ...
    <aside className="w-60 shrink-0 flex flex-col bg-[#120000] border-r border-white/[0.06] min-h-screen">
      <div className="px-5 py-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-white text-sm font-semibold leading-none">Oberlin Calendar</p>
            <p className="text-zinc-500 text-[11px] mt-0.5">Research Dashboard</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {[...nav, ...(isAdmin ? [{
          ...
          const count = item.badge && item.badge !== "soon" ? counts[item.badge as keyof typeof counts] : 0;
          return (
            <Link
              key={item.href}
              href={item.href}
