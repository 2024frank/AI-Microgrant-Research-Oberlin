/**
 * Oberlin Public Library Sync Pipeline — oberlin_library_v1
 *
 * Calls the Oberlin Library ingester, then runs every staged event through:
 ...
 *
 * Run: node --env-file=.env sync-oberlin-library.js
 */

import { initializeApp, cert, getApps } from "firebase-admin/app";
import { getFirestore } from "firebase-admin/firestore";
import { runIngester } from "./ingest-oberlin-library.js";
import {
  mightBeDuplicate,
...
const POST_TYPE_MAP = {
  ...
  "recreation": 12, "networking": 13, "storytime": 7,
  "story time": 7, "reading": 6, "book": 6, "author": 6,
  "craft": 7, "children": 7, "teen": 7, "youth": 7,
  "summer reading": 7, "club": 13,
};
...
async function buildPayloadFromStaged(ev) {
  ...
  const payload = {
    ...
    sponsors:            [ev.organizational_sponsor || ev.source_name || "Oberlin Public Library"],
...
- Internal library staff events

PUBLIC (approve) if:
- Open to the Oberlin community or general public
- Library programs: storytime, book clubs, workshops, author events
- All-ages or family-friendly events

Note: Public library events are almost always open to the public. Default to PUBLIC for library programs.
...
async function loadProcessedFingerprints(db) {
  ...
  try {
    const [queueSnap, rejectedSnap] = await Promise.all([
      db.collection("review_queue").where("source_id", "==", "oberlin_library").select("fingerprint").get(),
      db.collection("rejected").where("source_id", "==", "oberlin_library").select("fingerprint").get(),
    ]);
    const fps = new Set();
...
async function fetchCommunityHubEvents() {
  try {
    const res = await fetch(COMMUNITYHUB_POSTS_API, {
      headers: { "User-Agent": "oberlin-library-sync-bot/1.0" },
    });
...
async function main() {
  ...
  console.log("→ Running Oberlin Public Library ingester…\n");
  ...
  for (let i = 0; i < stagedEvents.length; i++) {
    ...
    const alreadyQueued = await checkDuplicateInQueue(db, incoming, "oberlin_library");
  ...
  if (db) {
    await db.collection("syncs").doc("oberlin_library").set({
