/**
 * Experience Oberlin Sync Pipeline — experience_oberlin_v1
 *
 * Calls the Experience Oberlin ingester, then runs every staged event through:
 ...
 *
 * Run: node --env-file=.env sync-experience-oberlin.js
 */

import { initializeApp, cert, getApps } from "firebase-admin/app";
import { getFirestore } from "firebase-admin/firestore";
import { runIngester } from "./ingest-experience-oberlin.js";
import {
  mightBeDuplicate,
...
async function buildPayloadFromStaged(ev) {
  ...
  const payload = {
    ...
    sponsors:            [ev.organizational_sponsor || ev.source_name || "Experience Oberlin"],
...
async function loadProcessedFingerprints(db) {
  ...
  try {
    const [queueSnap, rejectedSnap] = await Promise.all([
      db.collection("review_queue").where("source_id", "==", "experience_oberlin").select("fingerprint").get(),
      db.collection("rejected").where("source_id", "==", "experience_oberlin").select("fingerprint").get(),
    ]);
    const fps = new Set();
...
async function fetchCommunityHubEvents() {
  try {
    const res = await fetch(COMMUNITYHUB_POSTS_API, {
      headers: { "User-Agent": "experience-oberlin-sync-bot/1.0" },
    });
...
async function main() {
  ...
  console.log("→ Running Experience Oberlin ingester…\n");
  ...
  for (let i = 0; i < stagedEvents.length; i++) {
    ...
    const alreadyQueued = await checkDuplicateInQueue(db, incoming, "experience_oberlin");
  ...
  if (db) {
    await db.collection("syncs").doc("experience_oberlin").set({
