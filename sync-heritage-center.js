/**
 * Heritage Center Sync Pipeline — heritage_center_v1
 *
 * Calls the Heritage Center ingester, then runs every staged event through:
 ...
 *
 * Run: node --env-file=.env sync-heritage-center.js
 */

import { initializeApp, cert, getApps } from "firebase-admin/app";
import { getFirestore } from "firebase-admin/firestore";
import { runIngester } from "./ingest-heritage-center.js";
import {
  mightBeDuplicate,
...
async function buildPayloadFromStaged(ev) {
  ...
  const payload = {
    ...
    sponsors:           [ev.organizational_sponsor || ev.source_name || "Oberlin Heritage Center"],
...
- Public lectures, tours, workshops, exhibitions, open houses, festivals
...
async function loadProcessedFingerprints(db) {
  ...
  try {
    const [queueSnap, rejectedSnap] = await Promise.all([
      db.collection("review_queue").where("source_id", "==", "heritage_center").select("fingerprint").get(),
      db.collection("rejected").where("source_id", "==", "heritage_center").select("fingerprint").get(),
    ]);
    const fps = new Set();
...
async function fetchCommunityHubEvents() {
  try {
    const res = await fetch(COMMUNITYHUB_POSTS_API, {
      headers: { "User-Agent": "heritage-center-sync-bot/1.0" },
    });
...
async function main() {
  ...
  console.log("→ Running Heritage Center ingester…\n");
  ...
  for (let i = 0; i < stagedEvents.length; i++) {
    ...
    const alreadyQueued = await checkDuplicateInQueue(db, incoming, "heritage_center");
  ...
  if (db) {
    await db.collection("syncs").doc("heritage_center").set({
