/**
 * FAVA Gallery Sync Pipeline — fava_v1
 *
 * Calls the FAVA Gallery ingester, then runs every staged event through:
 ...
 *
 * Run: node --env-file=.env sync-fava.js
 */

import { initializeApp, cert, getApps } from "firebase-admin/app";
import { getFirestore } from "firebase-admin/firestore";
import { runIngester } from "./ingest-fava.js";
import {
  mightBeDuplicate,
...
const POST_TYPE_MAP = {
  ...
  "recreation": 12, "networking": 13, "art": 2, "photography": 2,
  "ceramics": 7, "pottery": 7, "painting": 2, "drawing": 2,
  "printmaking": 2, "sculpture": 2,
};
...
async function buildPayloadFromStaged(ev) {
  ...
  const payload = {
    ...
    sponsors:            [ev.organizational_sponsor || ev.source_name || "FAVA — Fine Arts Association"],
...
- Arts classes, workshops, gallery openings open to the public
...
async function loadProcessedFingerprints(db) {
  ...
  try {
    const [queueSnap, rejectedSnap] = await Promise.all([
      db.collection("review_queue").where("source_id", "==", "fava").select("fingerprint").get(),
      db.collection("rejected").where("source_id", "==", "fava").select("fingerprint").get(),
    ]);
    const fps = new Set();
...
async function fetchCommunityHubEvents() {
  try {
    const res = await fetch(COMMUNITYHUB_POSTS_API, {
      headers: { "User-Agent": "fava-sync-bot/1.0" },
    });
...
async function main() {
  ...
  console.log("→ Running FAVA Gallery ingester…\n");
  ...
  for (let i = 0; i < stagedEvents.length; i++) {
    ...
    const alreadyQueued = await checkDuplicateInQueue(db, incoming, "fava");
  ...
  if (db) {
    await db.collection("syncs").doc("fava").set({
      source:            "FAVA — Fine Arts Association",
      queued,
      skipped,
